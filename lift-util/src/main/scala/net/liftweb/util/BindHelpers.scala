...
trait BindHelpers {
  ...
  /**
   * Allow bind by a list of bindParams, followed by a varArg list, to support bindByName.<p/>
   *
   * @param namespace the namespace of tags to bind
   * @param xml the NodeSeq in which to find elements to be bound.
   * @param params1 the list of BindParam bindings to be applied
   * @param params2 the vararg array of BindParam bindings to be applied
   *
   * @return the NodeSeq that results from the specified transforms
   */
  // allow bind by a list of bindParams, followed by a varArg list, to support bindByName
  def bind(namespace: String, xml: NodeSeq, params1: List[BindParam], params2: BindParam*): NodeSeq = {
      val allParams = params1 ::: params2.toList
      bind(namespace, xml, allParams.toArray[BindParam]: _*)
  }
