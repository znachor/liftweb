...
class JodaTimeHelpersTest extends JUnit4(TimeHelpersSpec)
object JodaTimeHelpersSpec extends Specification with ScalaCheck with Mocker with LoggerDelegation with ControlHelpers with ClassHelpers {
  def testEqualJodaSpan (first : JodaSpan, second : JodaSpan) = first must_== second

  "A JodaSpan" can {
    "be created from a number of milliseconds" in {
      JodaSpan(3000) must_== JodaSpan(3 * 1000)
    }
    "be created from a number of seconds" in {
      testEqualJodaSpan(3.seconds, JodaSpan(3 * 1000))
    }
    "be created from a number of minutes" in {
      testEqualJodaSpan(3.minutes, JodaSpan(3 * 60 * 1000))
    }
    "be created from a number of hours" in {
      testEqualJodaSpan(3.hours, JodaSpan(3 * 60 * 60 * 1000))
    }
    "be created from a number of days" in {
      testEqualJodaSpan(3.days, JodaSpan(3 * 24 * 60 * 60 * 1000))
    }
    "be created from a number of weeks" in {
      testEqualJodaSpan(3.weeks, JodaSpan(3 * 7 * 24 * 60 * 60 * 1000))
    }
  ...
  "A JodaSpan" should {
    ...
    "have a later method returning a date relative to now plus the time span" in {
      3.seconds.later.millis must beCloseTo((now + 3.seconds).millis, 100L)
    }
    "have an ago method returning a date relative to now minus the time span" in {
      3.seconds.ago.millis must beCloseTo((now - 3.seconds).millis, 100L)
    }
  ...
  "the JodaTimeHelpers" should {
    ...
    "provide a toDate returning a Full(DateTime) from many kinds of objects" in {
      ...
      toDate(internetDateFormatter.format(d)).get.millis must beCloseTo(d.millis, 1000L)
    }
  }
