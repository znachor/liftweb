...
object CouchRecordTestSpecs extends Specification {
    ...
    "A couch record" should {
        ...
        "be insertable" in {
            ...
            val Full(foundRec) = Person.fetch(newRec.id.value.open_!)
        ...
        "generate the right JSON" in {
            ...
            val foundDoc = Http(defaultDatabase(newRec.id.value.open_!) fetch)
        ...
        "be deletable" in {
            ...
            newRec.save
            Http(defaultDatabase(newRec.id.value.open_!) @@ newRec.rev.value.open_! delete)
            newRec.delete_! must not(verify(_.isDefined))
        }
        ...
        "be fetchable in bulk" in {
            ...
            Person.fetchMany(newRec1.id.value.open_!, newRec3.id.value.open_!).map(_.toList) must beLike {
        ...
        "support multiple databases for fetching" in {
            ...
            val foundRecBox = Person.fetchFrom(database2, newRec.id.value.open_!)
            foundRecBox must verify(_.isDefined)
            val Full(foundRec) = foundRecBox
            assertEqualPerson(foundRec, testRec1)
            foundRec.id.value must_== newRec.id.value
            ...
            Person.fetch(newRec.id.value.open_!) must not(verify(_.isDefined))
        }
        ...
        "support multiple databases for fetching in bulk" in {
            ...
            Person.fetchManyFrom(database2, newRec1.id.value.open_!, newRec3.id.value.open_!).map(_.toList) must beLike {
                case Full(foundRows) => assertEqualRows(foundRows, expectedRows); true
            }

            Person.fetchMany(newRec1.id.value.open_!, newRec3.id.value.open_!) must beLike { case Full(seq) if seq.isEmpty => true }
        }
